...
SELECT name, SUM(speed) AS faster FROM marvels GROUP BY gender;

SELECT name, AVG(speed) AS avg_faster FROM marvels GROUP BY gender HAVING avg_faster > 3; 

SELECT name,  popularity, ROUND(speed * 100) AS faster FROM marvels GROUP BY gender;


SELECT name, COUNT(*) AS hero_count,
    CASE
        WHEN strength > speed THEN "Powerfull"
        WHEN strength < speed THEN "Swift"
        WHEN strength = speed THEN "Formidable"
        ELSE "Weak"
    END AS "capacity"
FROM marvels
GROUP BY capacity;
...
SELECT name FROM marvels WHERE hometown LIKE "USA" AND alignment LIKE "Neutral"
SELECT name 